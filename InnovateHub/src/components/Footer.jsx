import React from 'react';
import { Link } from 'react-router-dom';
import PropTypes from 'prop-types';

/**
 * Footer component with links and copyright information
 * @param {Object} props - Component props
 * @param {string} props.copyrightText - Copyright text
 * @param {Array} props.links - Footer links
 * @param {Array} props.socialLinks - Social media links
 * @param {string} props.className - Additional CSS classes
 * @returns {JSX.Element} - Footer component
 */
const Footer = ({ 
  copyrightText = `© ${new Date().getFullYear()} PLP Task Manager. All rights reserved.`,
  links = [
    { text: 'Home', href: '/' },
    { text: 'About', href: '/about' },
    { text: 'Privacy Policy', href: '#' },
    { text: 'Terms of Service', href: '#' },
  ],
  socialLinks = [
    { icon: 'github', href: 'https://github.com/WambuiGrace' },
    { icon: 'linkedin', href: 'https://www.linkedin.com/in/grace-mwangi-4201a4240/' },
  ],
  className = '',
  ...rest 
}) => {
  // Social media icons
  const socialIcons = {
    github: (
      <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
        <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
      </svg>
    ),
    linkedin: (
      <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
      </svg>
    ),
  };

  return (
    <footer className={`bg-white dark:bg-gray-800 shadow ${className}`} {...rest}>
      <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        <div className="md:flex md:items-center md:justify-between">
          <div className="flex justify-center space-x-6 md:order-2">
            {socialLinks.map((link, index) => (
              <a 
                key={index} 
                href={link.href} 
                className="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300"
                target="_blank"
                rel="noopener noreferrer"
              >
                <span className="sr-only">{link.icon}</span>
                {socialIcons[link.icon]}
              </a>
            ))}
          </div>
          <div className="mt-8 md:mt-0 md:order-1">
            <p className="text-center text-base text-gray-500 dark:text-gray-400">
              {copyrightText}
            </p>
          </div>
        </div>
        <div className="mt-8 border-t border-gray-200 dark:border-gray-700 pt-8 flex flex-wrap justify-center space-x-6">
          {links.map((link, index) => (
            link.href.startsWith('/') ? (
              <Link 
                key={index} 
                to={link.href} 
                className="text-gray-500 hover:text-gray-600 dark:text-gray-400 dark:hover:text-gray-300 text-sm"
              >
                {link.text}
              </Link>
            ) : (
              <a 
                key={index} 
                href={link.href} 
                className="text-gray-500 hover:text-gray-600 dark:text-gray-400 dark:hover:text-gray-300 text-sm"
              >
                {link.text}
              </a>
            )
          ))}
        </div>
      </div>
    </footer>
  );
};

Footer.propTypes = {
  copyrightText: PropTypes.string,
  links: PropTypes.arrayOf(
    PropTypes.shape({
      text: PropTypes.string.isRequired,
      href: PropTypes.string.isRequired,
    })
  ),
  socialLinks: PropTypes.arrayOf(
    PropTypes.shape({
      icon: PropTypes.oneOf(['github', 'linkedin']).isRequired,
      href: PropTypes.string.isRequired,
    })
  ),
  className: PropTypes.string,
};

export default Footer;